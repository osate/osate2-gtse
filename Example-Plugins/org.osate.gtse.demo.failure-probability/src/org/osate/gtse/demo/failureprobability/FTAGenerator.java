...
/*
 * Copy of org.osate.aadl2.errormodel.faulttree.generation.FTAGenerator
 * modified to call PFDUtil to fill failure probabilities
 */
class FTAGenerator extends PropagationGraphBackwardTraversal {
	private FaultTree ftaModel;

	FTAGenerator(PropagationGraph currentPropagationGraph) {
		super(currentPropagationGraph);
		ftaModel = null;
		FaultTreeUtils.resetIntermediateEventCount();
	}

	FaultTree getftaModel(ComponentInstance rootComponent, NamedElement rootStateOrPropagation,
...
//			xformXORtoOR(emftaRootEvent);
			...
			PFDUtil.fillProbabilities(ftaModel);
