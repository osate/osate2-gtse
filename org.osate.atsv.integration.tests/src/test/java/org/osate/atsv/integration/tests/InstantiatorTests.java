...
//public class InstantiatorTests extends XtextTest {
//
//	@Inject
...
//
//	private static final String FILE = "org.osate.atsv.integration.tests/src/test/resources/IntegrationTestsModel.aadl";
//
...
//				} else {
//
//					Property property = Aadl2GlobalScopeUtil.get(ci, Aadl2Package.eINSTANCE.getProperty(), prop);
//					PropertyExpression pe = ci.getNonModalPropertyValue(property);
//
//					if (value instanceof Collection) {
////						Iterator<PropertyExpression> propIter = propVals.iterator();
//						@SuppressWarnings("unchecked")
...
public class InstantiatorTests extends OsateTest {
		...
		while (cisIter.hasNext()) {
			...
			if (ci.getName().equalsIgnoreCase(pathSegment)) {
				if (pathIter.hasNext()) {
					...
					List<PropertyExpression> propVals = getPropertyValues(ci, prop.split("\\:\\:")[0],
							prop.split("\\:\\:")[1]);
	...
	*/
	...
	private EList<PropertyExpression> getPropertyValues(NamedElement namedElement, String propertySetName,
			String propertyName) {
		final EList<PropertyExpression> result = new BasicEList<PropertyExpression>();
		final Property property = lookupPropertyDefinition(namedElement, propertySetName, propertyName);
	...
	private Property lookupPropertyDefinition(EObject context, String propSetName, String propName) {
